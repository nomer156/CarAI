...
function App() {
  ...
  useEffect(() => {
    ...
    QRCode.toDataURL(code, {
      width: 176,
      margin: 1,
      color: { dark: '#173042', light: '#0000' },
    }).then(setOwnerQrCode).catch(() => setOwnerQrCode(''));
  }, [state.vehicle.ownerCode, session?.user?.id]);
  useEffect(() => {
  ...
  async function setQueueStatus(queueId: string, status: 'new' | 'confirmed' | 'in_service' | 'ready') {
    if (session && hasCloudProfile && (state.role === 'mechanic' || state.role === 'service_admin')) {
      try {
        ...
        setSyncStatus(error instanceof Error ? error.message : 'Не удалось обновить статус очереди.');
      }
      return;
    }
